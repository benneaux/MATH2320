...
3.	Consider the list of vectors $B = ((1, 0, 0), (1, 1, 0), (1, 1, 1))$ from the vector space $\mathbb{R}^3$.
    ...
    **Answer:** The vector $\vec{t} \in span(B): \vec{t} = (9,8,16)$ can be defined as a combination of three vectors $\vec{u},\vec{v},\vec{w},\in B: \vec{u} = (1,0,0), \vec{v} = (1,1,0) \text{ and } \vec{w} = (1,1,1)$ by the relationship $\vec{t} = (16\cdot \vec{w}) - (8 \cdot \vec{v}) + (1 \cdot \vec{u})$
    The list of vectors $B$ does span $\mathbb{R}^3$ because, using the vectors $\vec{u},\vec{v},\vec{w},\in B$ defined above: $$\begin{align} &\forall \vec{t} \in \mathbb{R}^3\left\{ \vec{t} := (\alpha,\beta,\gamma): \alpha,\beta,\gamma \in \mathbb{F} \right\} \\ \implies &\forall \vec{u},\vec{v},\vec{w},\in B, \vec{t} := ((\gamma)\cdot\vec{w}) + ((\beta-\gamma)\cdot\vec{v}) + ((\alpha-\beta)\cdot\vec{u}) \end{align}$$ 
